            ...
            with col1:
                if st.button("Generate Needs Matrix", type="primary"):
                    ...
                    def update_progress(current, total, name):
                        ...
                        progress_bar.progress(percent, text=f"Analyzing {current}/{total}: {name}")
                    
                    with st.spinner("Analyzing student requirements..."):
                        from logic.course_matching import generate_needs_matrix, get_course_last_offered
                        needs_df = generate_needs_matrix(students_df, progress_callback=update_progress)
            ...
            if 'needs_df' in st.session_state:
                ...
                for course in course_cols:
                    ...
                    if count > 0:
                        ...
                        months_ago = history['MonthsAgo'] if history else 999 # Never offered?
                ...
                st.write("Prioritize courses with high demand (>16) and long time since last offered.")
                ...
                if not demand_df.empty:
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "Demand": st.column_config.ProgressColumn("Student Demand", min_value=0, max_value=max(demand_df['Demand']), format="%d"),
                        }
                    )
                else:
        ...
        if 'final_schedule' in st.session_state and 'needs_df' in st.session_state:
            ...
            if affected_students:
                ...
                with col2:
                    ...
                    telegram_chat_id = st.text_input("Telegram Channel/Group ID", placeholder="@channel_name or -100xxxxx")
